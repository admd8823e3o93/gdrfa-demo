...
// Prefer env override for uploads so they can live on Render disk
const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(ROOT, "uploads");
fs.mkdirSync(UPLOAD_DIR, { recursive: true });

// Support either "Public" or "public" (Linux is case-sensitive)
const publicCandidates = [path.join(ROOT, "Public"), path.join(ROOT, "public")];
const PUBLIC_DIR = publicCandidates.find((p) => fs.existsSync(p)) || publicCandidates[0];
// If not present, create it (helps on clean deployments)
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, db: DB_FILE, uploads: UPLOAD_DIR });
});
